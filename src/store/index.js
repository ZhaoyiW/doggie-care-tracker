...
const useStore = create((set) => ({
  ...
  updateFoodLog: (id, data) => {
    ...
    set(s => ({ foodLogs: s.foodLogs.map(f => f.id === id ? { ...f, ...record } : f) }))
    api.post('food-logs', record)
  },
  deleteFoodLog: (id) => {
  ...
  updatePoopLog: (id, data) => {
    const record = { id, ...data }
    set(s => ({ poopLogs: s.poopLogs.map(p => p.id === id ? { ...p, ...record } : p) }))
    api.post('poop-logs', record)
  },
  deletePoopLog: (id) => {
  ...
  updateSymptomLog: (id, data) => {
    const record = { id, ...data }
    set(s => ({ symptomLogs: s.symptomLogs.map(sl => sl.id === id ? { ...sl, ...record } : sl) }))
    api.post('symptom-logs', record)
  },
  deleteSymptomLog: (id) => {
  ...
  updateVaccineRecord: (id, data) => {
    const record = { id, ...data }
    set(s => ({ vaccineRecords: s.vaccineRecords.map(v => v.id === id ? { ...v, ...record } : v) }))
    api.post('vaccine-records', record)
  },
  ...
  updateVetVisit: (id, data) => {
    const record = { id, ...data }
    set(s => ({ vetVisits: s.vetVisits.map(v => v.id === id ? { ...v, ...record } : v) }))
    api.post('vet-visits', record)
  },
  deleteVetVisit: (id) => {
  ...
  updateHealthTest: (id, data) => {
    const record = { id, ...data }
    set(s => ({ healthTests: s.healthTests.map(t => t.id === id ? { ...t, ...record } : t) }))
    api.post('health-tests', record)
  },
  deleteHealthTest: (id) => {
  ...
  updateDewormingRecord: (id, data) => {
    const record = { id, ...data }
    set(s => ({ dewormingRecords: s.dewormingRecords.map(r => r.id === id ? { ...r, ...record } : r) }))
    api.post('deworming-records', record)
  },
  ...
  updateBathLog: (id, data) => {
    const record = { id, ...data }
    set(s => ({ bathLogs: s.bathLogs.map(b => b.id === id ? { ...b, ...record } : b) }))
    api.post('bath-logs', record)
  },
  deleteBathLog: (id) => {
  ...
  updateVet: (id, data) => {
    const record = { id, ...data }
    set(s => ({ vets: s.vets.map(v => v.id === id ? { ...v, ...record } : v) }))
    api.post('vets', record)
  },
  deleteVet: (id) => {
  ...
  updateAppointment: (id, data) => {
    const record = { id, ...data }
    set(s => ({ appointments: s.appointments.map(a => a.id === id ? { ...a, ...record } : a) }))
    api.post('appointments', record)
  },
